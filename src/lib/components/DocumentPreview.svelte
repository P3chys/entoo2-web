...
<Modal isOpen={!!document} onClose={handleClose} maxWidth="5xl">
	...
	<div class="flex-1 overflow-auto p-4">
		...
		{:else if document.mime_type.includes('text')}
			<pre
				class="bg-surface-100 dark:bg-surface-800 p-4 rounded border border-surface-200 dark:border-surface-700 overflow-auto max-h-[calc(90vh-200px)] text-sm font-mono whitespace-pre-wrap">{textContent}</pre>
		{/if}
	</div>

	<!-- Footer -->
	<div
		class="flex items-center justify-end gap-2 p-4 border-t border-surface-200 dark:border-surface-700"
	>
