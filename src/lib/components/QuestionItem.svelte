<script lang="ts">
	...
	async function handleSubmitReply() {
		...
		try {
			...
			const { data, error } = await api.postFormData<Answer>(
...
<div class="card p-5 border-l-4 border-l-accent-primary relative">
	...
	>
		<div class="flex-1 min-w-0 mr-2">
			{#if !isExpanded}
				...
				<div
					class="text-sm font-medium text-light-text-primary dark:text-dark-text-primary mb-1 line-clamp-2"
				>
					{question.content}
				</div>
				<div
					class="flex items-center gap-3 text-xs text-light-text-tertiary dark:text-dark-text-tertiary"
				>
					<div class="flex items-center gap-1">
						<div
							class="w-5 h-5 rounded-full bg-gradient-to-br from-accent-primary to-accent-secondary flex items-center justify-center text-white font-bold text-xs"
						>
							{question.user?.email ? question.user.email[0].toUpperCase() : 'A'}
						</div>
						<span
							>{question.user?.email
								? question.user.display_name || question.user.email.split('@')[0]
								: 'Anonymous'}</span
						>
					</div>
			...
			{:else}
				...
				<div class="flex items-center gap-2 mb-1">
					<div
						class="w-8 h-8 rounded-full bg-gradient-to-br from-accent-primary to-accent-secondary flex items-center justify-center text-white font-bold text-sm"
					>
						{question.user?.email ? question.user.email[0].toUpperCase() : 'A'}
					</div>
					<div>
						...
						<div class="text-xs text-light-text-tertiary dark:text-dark-text-tertiary">
							{safeFormatDate(question.created_at)} · {answers.length}
							{answers.length === 1
								? $_('common.answer') || 'answer'
								: $_('common.answers') || 'answers'}
						</div>
					</div>
				</div>
			{/if}
		...
		<Icon
			...
			size={20}
			className="text-light-text-tertiary dark:text-dark-text-tertiary flex-shrink-0 mt-1"
		/>
	</button>
	...
	{#if isExpanded}
		<!-- Question Content -->
		<div
			class="prose dark:prose-invert max-w-none text-light-text-secondary dark:text-dark-text-secondary mb-4 break-words"
			transition:slide
		>
			{question.content}
		</div>
		...
		<!-- Actions Bar -->
		<div
			class="flex items-center justify-between border-t border-light-border-primary dark:border-dark-border-primary pt-3 mt-2"
		>
					...
					<div class="flex items-center justify-between">
						<div class="flex items-center gap-2 flex-1 min-w-0">
							{#if replyFile}
								...
								<div
									class="flex items-center gap-2 bg-green-50 dark:bg-green-900/20 border border-green-300 dark:border-green-700 rounded px-2 py-1.5 flex-1 min-w-0"
								>
									<Icon
										name="file"
										size={14}
									...
									<div class="flex-1 min-w-0">
										<div
											class="text-xs font-medium text-green-700 dark:text-green-300 truncate"
		...
		{#if answers.length > 0}
			<div
				class="mt-4 space-y-4 pl-4 border-l-2 border-light-border-primary dark:border-dark-border-primary"
			>
					...
					<div class="bg-light-bg-tertiary dark:bg-dark-bg-tertiary rounded-lg p-3">
						<div class="flex items-center gap-2 mb-2">
							<div class="font-medium text-xs text-light-text-primary dark:text-dark-text-primary">
								{answer.user?.email
									? answer.user.display_name || answer.user.email.split('@')[0]
									: 'User'}
							</div>
							<div class="text-xs text-light-text-tertiary dark:text-dark-text-tertiary">
						...
						<div
							class="text-sm text-light-text-secondary dark:text-dark-text-secondary break-words mb-2"
						>
							{answer.content}
						</div>
						...
						{#if answer.document}
							<div
								class="flex items-center gap-2 bg-light-bg-primary dark:bg-dark-bg-primary p-2 rounded border border-light-border-primary dark:border-dark-border-primary max-w-fit"
							>
								<Icon name="file" size={16} className="text-accent-primary" />
								<span class="text-xs font-medium truncate max-w-[150px]"
									>{answer.document.original_name}</span
								>
								<button
