<script lang="ts">
	...
	async function loadQuestions() {
		loading = true;
		const res = await api.get<{ success: boolean; data: Question[] }>(
			`/api/v1/subjects/${subjectId}/questions`
		);
		if (res.data?.success) {
	...
	async function handleSubmit() {
		...
		submitting = true;
		const res = await api.post<{ success: boolean; data: Question }>(
			`/api/v1/subjects/${subjectId}/questions`,
			{
...
<div class="space-y-6">
			...
			<div class="flex items-center justify-between">
				<label class="flex items-center gap-2 text-sm cursor-pointer select-none">
					<input
						...
						class="rounded text-accent-primary focus:ring-accent-primary bg-light-bg-tertiary dark:bg-dark-bg-tertiary border-light-border-primary dark:border-dark-border-primary"
					/>
	...
	{:else if questions.length === 0}
		<div
			class="text-center py-12 bg-light-bg-secondary dark:bg-dark-bg-secondary rounded-xl border-2 border-dashed border-light-border-primary dark:border-dark-border-primary"
		>
			<Icon
				name="chat"
				size={48}
				className="text-light-text-tertiary dark:text-dark-text-tertiary mx-auto mb-3"
			/>
			<p class="text-light-text-secondary dark:text-dark-text-secondary">
				{$_('questions.empty') || 'No questions yet. Be the first to ask!'}
			</p>
		</div>
	{:else}
		<div class="space-y-4">
