<script lang="ts">
	...
	async function toggleFavoriteSubject(subject: Subject) {
		...
		const { error } = await api.post<{ success: boolean; is_favorite: boolean }>(
	...
	async function removeFavoriteDocument(doc: Document) {
		...
		const { error } = await api.post<{ success: boolean; is_favorite: boolean }>(
			`/api/v1/documents/${doc.id}/favorite`,
			{}
		);
...
<div class="space-y-8" in:fade={{ duration: 200 }}>
	...
	{:else if favoriteSubjects.length === 0 && favoriteDocuments.length === 0}
		<div
			class="text-center py-16 bg-surface-50 dark:bg-surface-800 rounded-lg border border-surface-200 dark:border-surface-700"
		>
			<div
				class="inline-flex items-center justify-center w-16 h-16 rounded-full bg-surface-100 dark:bg-surface-700 mb-4"
			>
	...
	{:else}
				...
				<div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
					{#each favoriteSubjects as subject (subject.id)}
						...
						>
							<div class="flex justify-between items-start">
								<div>
									<div class="flex items-center gap-2 mb-2">
										<span
											class="font-mono font-bold bg-surface-200 dark:bg-surface-700 px-2 py-1 rounded text-xs"
										>
											{subject.code}
										</span>
									</div>
									<h3
										class="font-bold text-lg mb-1 group-hover:text-primary-600 dark:group-hover:text-primary-400 transition-colors"
									>
										{subject.name_cs}
									</h3>
							...
							{#if subject.semester}
								<div
									class="mt-4 text-xs text-surface-500 border-t border-surface-200 dark:border-surface-700 pt-2"
								>
		...
		{#if favoriteDocuments.length > 0}
			<section>
				...
				<div
					class="bg-surface-50 dark:bg-surface-800 rounded-lg border border-surface-200 dark:border-surface-700 p-4"
				>
					<div class="space-y-2">
						{#each favoriteDocuments as doc (doc.id)}
							<div
								class="flex items-center justify-between p-3 bg-light-bg-tertiary dark:bg-dark-bg-tertiary rounded-lg hover:shadow-md transition-shadow"
							>
								<div class="flex items-center gap-3 flex-1 min-w-0">
									...
									<div class="flex-1 min-w-0">
										...
										{#if doc.subject}
											<div
												class="text-xs text-light-text-secondary dark:text-dark-text-secondary truncate"
											>
