	...
	{:else if subject}
		...
		<div class="mb-6 card p-6" use:slideInFrom={{ direction: 'top', delay: 100 }}>
			<div class="flex items-center gap-3 mb-2 flex-wrap">
				<span
					class="font-mono font-bold bg-accent-primary/10 text-accent-primary px-3 py-1.5 rounded-lg text-sm"
				>
					{subject.code}
				</span>
				...
				<span
					class="text-xs bg-success/10 text-success px-3 py-1 rounded-full font-medium flex items-center gap-1"
				>
		...
		<!-- Tabs -->
		<div
			class="flex border-b border-light-border-primary dark:border-dark-border-primary mb-6 overflow-x-auto"
			use:fadeSlideIn={{ delay: 200 }}
		>
			{#each tabs as tab}
				<button
					class="px-6 py-3 font-medium text-sm transition-all duration-200 relative whitespace-nowrap flex items-center gap-2 {activeTab ===
					tab.id
						? 'text-accent-primary'
				...
				>
					...
					{#if activeTab === tab.id}
						<div
							class="absolute bottom-0 left-0 w-full h-[3px] bg-accent-primary"
		...
		<div class="min-h-[400px]">
					...
					<div class="card p-6 space-y-6">
						<div>
							...
							{#if subject.description_cs}
								<div
									class="prose dark:prose-invert max-w-none text-light-text-secondary dark:text-dark-text-secondary text-sm"
								>
							...
							<div>
								...
								<div class="grid grid-cols-1 md:grid-cols-2 gap-3" use:staggerFadeIn>
									{#each subject.teachers as teacher}
										<div
											class="text-sm bg-light-bg-tertiary dark:bg-dark-bg-tertiary px-4 py-3 rounded-lg border border-light-border-primary dark:border-dark-border-primary hover:shadow-md transition-shadow"
										>
											<div class="flex items-start gap-3 mb-2">
												...
												<div class="flex flex-col flex-1">
													...
													{#if teacher.topic_cs}
														<span
															class="text-light-text-tertiary dark:text-dark-text-tertiary text-xs"
															>{teacher.topic_cs}</span
														>
													{/if}
												</div>
											</div>
				...
				<div class="space-y-6" in:fadeSlideIn={{ delay: 100 }}>
					<div class="flex items-center justify-between">
						...
						<div class="flex items-center gap-3">
							{#if isAdmin}
								<Button variant="ghost" size="sm" onclick={() => (showCategoryManager = true)}>
									<Icon name="settings" size={16} />
									{$_('documents.manageCategories')}
								</Button>
							{/if}
							<span
								class="text-sm bg-light-bg-secondary dark:bg-dark-bg-secondary px-3 py-1 rounded-full border border-light-border-primary dark:border-dark-border-primary font-medium"
							>
								{documents.length}
					...
					<div class="card p-6">
						...
						<div
							class="mt-8 p-4 bg-light-bg-tertiary dark:bg-dark-bg-tertiary rounded-xl border border-dashed border-light-border-secondary dark:border-dark-border-secondary"
						>
							<FileUpload
								subjectId={subject.id}
			...
			{:else if activeTab === 'questions'}
				<div in:fadeSlideIn={{ delay: 100 }}>
					<div class="card p-6">
						<QuestionList subjectId={subject.id} currentUser={$user} />
					</div>
				</div>
			{:else if activeTab === 'messages'}
				<div class="grid grid-cols-1 lg:grid-cols-4 gap-6" in:fadeSlideIn={{ delay: 100 }}>
					...
					<div class="hidden lg:block lg:col-span-1">
						...
						<div
							class="bg-accent-primary/5 border border-accent-primary/20 rounded-lg p-4 text-sm text-light-text-secondary dark:text-dark-text-secondary"
						>
