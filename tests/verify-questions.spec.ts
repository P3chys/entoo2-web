...
test.describe('Verify Questions Implementation', () => {
  test('verify collapsible questions and bulk download features', async ({ page }) => {
    ...
    if (await cs101Link.count() > 0) {
      ...
      const parentCard = cs101Link.first().locator('xpath=ancestor::*[self::a or contains(@class, "cursor-pointer")]').first();
      ...
      if (tabClicked) {
        ...
        const questionCards = await page.locator('.card').filter({ has: page.locator('button') }).count();
        ...
        if (questionCards > 0) {
          ...
          const firstQuestion = page.locator('.card').filter({ has: page.locator('button') }).first();
